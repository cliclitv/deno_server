...
export default async (c: Context) => {
  ...
  switch (tag) {
    case 'hcy':
      ...
      const cookie: string = await fetch(`https://api.clicli.us/cookie/${info[0]}`)
        .then((resp) => resp.json())
    ...
    case '1096':
      ...
      url = await fetch(`https://qt.qq.com/php_cgi/cod_video/php/get_video_url.php?json=1&multirate=1&filetype=40&game_id=123456&vid=${vid}`)
        .then((resp) => resp.json())
